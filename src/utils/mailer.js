...
const sendConnectionRequestEmail = async (toUser, fromUser) => {
  ...
  const htmlMessage = `
    ...
    fromUser.lastName
      ...
      <p style="font-size: 13px; color: #888; text-align:center;">
        Stay connected with <strong>DevTinder</strong> and grow your network!
      </p>
...
const sendContactFormEmail = async (formData) => {
  ...
  try {
    ...
    if (error) {
      console.log("Resend contact form error:", error);
      return false;
    }
...
const sendWelcomeEmail = async (firstName, lastName, emailId) => {
  try {
    const { data, error } = await resend.emails.send({
      from: "DevTinder <onboarding@resend.dev>",
      to: emailId,
      subject: `Welcome to DevTinder, ${firstName}!`,
      html: `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 4px; border-radius: 20px;">
          <div style="background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.1);">
            ...
            <div style="background: #1f2937; padding: 30px; text-align: center; color: #9ca3af;">
              <p style="margin: 8px 0; font-size: 14px; font-weight: 500;">© ${new Date().getFullYear()} DevTinder</p>
              <p style="margin: 8px 0; font-size: 12px; color: #6b7280;">Connecting developers worldwide</p>
            </div>
          </div>
        </div>
      `,
