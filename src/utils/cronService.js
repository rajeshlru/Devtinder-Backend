...
class CronService {
  ...
  // Updated New Year Wishes method
  startNewYearWishes() {
    const job = cron.schedule("0 0 1 1 *", async () => {
      console.log("🎊 New Year cron job triggered");
  ...
  // Updated sendNewYearWishEmail using Resend
  ...
  // Updated Day 1 Onboarding Email
  ...
  // Updated Day 2 Onboarding Email
  ...
  // Updated Day 3 Onboarding Email
  ...
  async sendWeeklyDigestEmail(user) {
    ...
    const html = `
      <!DOCTYPE html>
      <html>
      <!-- Your existing weekly digest HTML content -->
      ${/* Paste your entire weekly digest HTML here */ ""}
      </html>
    `;
  ...
  async sendWeeklyAdminReport(
    ...
    const subject = `📊 Your Weekly DevTinder Insights - ${new Date().toDateString()}`;
  ...
  // Updated startWeeklyAdminReport method
  startWeeklyAdminReport() {
    const job = cron.schedule("0 8 * * 1", async () => {
      try {
        const totalUsers = await User.countDocuments();
  ...
  // Updated Inactive User Reminder
  ...
  // Updated startInactiveUserReminder method
  startInactiveUserReminder() {
    const job = cron.schedule("0 11 * * 0", async () => {
      try {
        const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
